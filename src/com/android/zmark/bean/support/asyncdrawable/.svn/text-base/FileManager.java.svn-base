...
public class FileManager {
	 ...
	 * cache dir. then open cache dir (/sdcard dir/Android/data/com.gogotown)
	 * with Root Explorer, uninstall weiciyuan and reinstall it, the new
	 * weiciyuan app will have the bug it can't read cache dir again, so I have
	 * to tell user to delete that cache dir
	 */
	private static volatile boolean cantReadBecauseOfAndroidBugPermissionProblem = false;

	private static String getSdCardPath() {
		if (isExternalStorageMounted()) {
			File path = ZmarkApplication.getInstance().getExternalCacheDir();
			if (path != null)
	...
	public File getAlbumStorageDir(String albumName) {
		File file = new File(
				...
				albumName);
		if (!file.mkdirs()) {
			LogHelper.e(TAG, "Directory not created");
		}
		return file;
	}
	...
	public static String getFilePathFromUrl(String url,
		...
		String oldRelativePath = s;
		if (s.indexOf("/") != -1)
			oldRelativePath = s.substring(s.indexOf("/"));
	...
	public static File createNewFileInSDCard(String absolutePath) {
		if (!isExternalStorageMounted()) {
			LogHelper.e(TAG, "sdcard unavailiable");
			return null;
		}
