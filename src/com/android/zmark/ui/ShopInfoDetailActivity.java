...
public class ShopInfoDetailActivity extends Activity implements OnClickListener{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		map= (Productlist) getIntent().getSerializableExtra("info");
	    context=this;
		this.init();
	}
	...
	public void init(){
		...
		//messagedetail_img=(LinearLayout) findViewById(R.id.messagedetail_img);
		...
		if(map!=null){
			messagedetail_info_title.setText(map.Name);
			messagedetail_from.setText("价格："+map.DanGrading+" "+map.Type);
			
			messagedetail_time.setText("");
			messagedetail_info.setText(map.Content);
		
			if( map.Img!=null && !DateInfo.IP.equals(DateInfo.IP+map.Img)){
			...
			if(ImageList!=null){
				getImage(ImageList);
				main_viewpager.setVisibility(View.VISIBLE);
			}else{
	...
	class Messagetask extends AsyncTask<String, Void, List<?>> {
		...
		protected List<?> doInBackground(String... arg0) {
			// TODO Auto-generated method stub
			return DateInfo.getDataFromSer(ShopInfoDetailActivity.this,DateInfo.GetMessage, null, "1",
					null, null);
		}
